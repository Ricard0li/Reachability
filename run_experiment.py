...
if (mode == 'all') or (mode == 'train'):
    ...
    p.add_argument('--device', type=str, default='auto', required=False, help='Device to use (cpu, cuda:0, mps).')
...
# Add device selection logic after the argument parsing
opt = p.parse_args()
...
# Set device based on input or auto-detect
if opt.device == 'auto':
...
if opt.device == 'mps' and not torch.backends.mps.is_available():
    ...
    opt.device = 'cpu'
elif opt.device.startswith('cuda') and not torch.cuda.is_available():
    print("Warning: CUDA device requested but not available. Falling back to CPU.")
    opt.device = 'cpu'

# Ensure the device is set correctly before using it
model.to(opt.device)
